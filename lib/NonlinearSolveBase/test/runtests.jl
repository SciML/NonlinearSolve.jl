...
# Used by the "maybe_wrap_nonlinear_f skips when a sparsity detector is supplied"
# testset below. Top-level so the struct definition is not nested inside
# `@testset`, which is unreliable across Julia versions.
struct _TestSparsityDetector <: ADTypes.AbstractSparsityDetector end
...
@testset "NonlinearSolveBase.jl" begin
    ...
    @testset "maybe_wrap_nonlinear_f skips when a sparsity detector is supplied" begin
        ...
        # ForwardDiff duals whose tag is generated by DI (`FixTail{...}`), not
        # `NonlinearSolveTag`. The wrapper's signatures are keyed off
        # `NonlinearSolveTag` and the foreign-tagged Dual is *isbits*, so
        # `AllowNonIsBits` does not fall back and the wrapper trips
        # `NoFunctionWrapperFoundError`. The fix is to leave the function
        # unwrapped whenever the user provides an `AbstractSparsityDetector`,
        # since sparsity-detector pre-passes intrinsically call the function
        # with eltypes the wrapper cannot cover.
        using NonlinearSolveBase, SciMLBase, ADTypes

        resid!(du, u, p) = (du .= u; nothing)

        # `NoSparsityDetector` (the default) must NOT inhibit wrapping.
        f_none = NonlinearFunction{true, SciMLBase.AutoSpecialize}(
            resid!; sparsity = ADTypes.NoSparsityDetector()
        )
        ...
        # A user-provided `AbstractSparsityDetector` must inhibit wrapping.
        f_sp = NonlinearFunction{true, SciMLBase.AutoSpecialize}(
            resid!; sparsity = _TestSparsityDetector()
        )
        prob_sp = NonlinearProblem(f_sp, [1.0, 2.0], [0.5, 0.25])
        @test NonlinearSolveBase.maybe_wrap_nonlinear_f(prob_sp) === f_sp.f
        @test !NonlinearSolveBase.is_fw_wrapped(
            NonlinearSolveBase.maybe_wrap_nonlinear_f(prob_sp)
        )
    end
