...
function _run_initialization!(cache, ::NonlinearSolveDefaultInit, prob, isinplace::Union{Val{true}, Val{false}})
    if SciMLBase.has_initialization_data(prob.f) && prob.f.initialization_data isa SciMLBase.OverrideInitData
        return _run_initialization!(cache, SciMLBase.OverrideInit(), prob, isinplace)
    end
    return cache, true
end

function _run_initialization!(cache, initalg::SciMLBase.OverrideInit, prob, isinplace::Union{Val{true}, Val{false}})
    ...
    u0, p, success = SciMLBase.get_initial_values(prob, cache, prob.f, initalg, isinplace; nlsolve_alg = alg, abstol = get_abstol(cache), reltol = get_reltol(cache))
...
get_abstol(prob::AbstractNonlinearProblem) = get_tolerance(get(prob.kwargs, :abstol, nothing), eltype(SII.state_values(prob)))
get_reltol(prob::AbstractNonlinearProblem) = get_tolerance(get(prob.kwargs, :reltol, nothing), eltype(SII.state_values(prob)))
...
function _run_initialization!(cache::AbstractNonlinearSolveCache, ::SciMLBase.NoInit, prob, isinplace)
    return cache, true
end
