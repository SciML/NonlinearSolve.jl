...
@testitem "Manual SCC" setup=[CoreRootfindTesting] tags=[:core] begin
    ...
    prob1 = NonlinearProblem(
        NonlinearFunction{true, SciMLBase.NoSpecialize}(f1), zeros(2), (cache, nothing))
    ...
    prob2 = NonlinearProblem(
        NonlinearFunction{true, SciMLBase.NoSpecialize}(f2), zeros(3), (cache, nothing))
    ...
    prob3 = NonlinearProblem(
        NonlinearFunction{true, SciMLBase.NoSpecialize}(f3), zeros(3), (cache, nothing))
    ...
    function SciMLBase.solve(prob::SciMLBase.SCCNonlinearProblem, alg; kwargs...)
        ...
        sols = [SciMLBase.build_solution(
                    prob, nothing, prob.u0, convert(eltype(u0), NaN) * prob.u0)
