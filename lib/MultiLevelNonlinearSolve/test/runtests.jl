...
run_tests(;
    ...
    groups = Dict(
        # Ferrite-backed demonstration of both variants on a real FEM problem. A dep-adding
        # group like QA: its own isolated sub-env, so Core stays free of the FEM stack.
        "Downstream" => (;
            env = joinpath(@__DIR__, "downstream"),
            body = function ()
                return @time @safetestset "Ferrite: Norton viscoelasticity, both variants" include("downstream/ferrite_norton.jl")
            end,
        ),
    ),
