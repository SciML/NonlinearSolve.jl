...
# A residual that is `Inf - Inf = NaN` at the initial guess. The NLLS default termination
# mode uses `L2_NORM`; when that carried `@fastmath`, the `!isfinite(objective)` protective
# break was optimized away and `TrustRegion` reported `ReturnCode.StalledSuccess` (a
# successful retcode) at `‖F‖ = NaN`.
