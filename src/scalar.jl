function SciMLBase.solve(prob::NonlinearProblem{<:Union{Number,SVector}}, alg::NewtonRaphson,
                         args...; xatol = nothing, xrtol = nothing, maxiters = 1000, kwargs...)
...
function SciMLBase.solve(prob::NonlinearProblem{<:Union{Number,SVector},iip,<:Dual{T,V,P}},
...
function SciMLBase.solve(prob::NonlinearProblem{<:Union{Number,SVector},iip,
                                                <:AbstractArray{<:Dual{T,V,P}}}, alg::NewtonRaphson,
...
for Alg in [Bisection]
    @eval function SciMLBase.solve(prob::NonlinearProblem{uType,iip,<:Dual{T,V,P}}, alg::$Alg,
                                   args...; kwargs...) where {uType,iip,T,V,P}
        sol, partials = scalar_nlsolve_ad(prob, alg, args...; kwargs...)
        return SciMLBase.build_solution(prob, alg, Dual{T,V,P}(sol.u, partials), sol.resid;
                                        retcode = sol.retcode,
    ...
    @eval function SciMLBase.solve(prob::NonlinearProblem{uType,iip,<:AbstractArray{<:Dual{T,V,P}}},
                                   alg::$Alg, args...; kwargs...) where {uType,iip,T,V,P}
        sol, partials = scalar_nlsolve_ad(prob, alg, args...; kwargs...)
        return SciMLBase.build_solution(prob, alg, Dual{T,V,P}(sol.u, partials), sol.resid;
                                        retcode = sol.retcode,
                                        left = Dual{T,V,P}(sol.left, partials),
...
function SciMLBase.solve(prob::NonlinearProblem, alg::Bisection, args...; maxiters = 1000,
    ...
    if iszero(fl)
        return SciMLBase.build_solution(prob, alg, left, fl; retcode = Symbol(EXACT_SOLUTION_LEFT),
    ...
    if !iszero(fr)
        while i < maxiters
            mid = (left + right) / 2
            (mid == left || mid == right) && return SciMLBase.build_solution(prob, alg, left, fl;
                                                                             retcode = Symbol(FLOATING_POINT_LIMIT), left = left,
                                                                             right = right)
            fm = f(mid)
            if iszero(fm)
                right = mid
