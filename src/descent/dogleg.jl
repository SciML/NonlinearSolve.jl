...
@concrete mutable struct DoglegCache{preinverted, normalform,
    NC <: NewtonDescentCache{preinverted, normalform},
    CC <: SteepestDescentCache{preinverted}}
    δu
    newton_cache::NC
    cauchy_cache::CC
    internalnorm
...
function SciMLBase.init(prob::NonlinearProblem, alg::NewtonDescent, J, fu, u;
        ...
        reltol = nothing, internalnorm::F = DEFAULT_NORM, kwargs...) where {F}
    @warn "Setting `preinverted = Val(true)` for `Dogleg` is not recommended." maxlog=1
    newton_cache = init(prob, alg.newton_descent, J, fu, u; preinverted, linsolve_kwargs,
        abstol, reltol, kwargs...)
    cauchy_cache = init(prob, alg.steepest_descent, J, fu, u; preinverted, linsolve_kwargs,
        abstol, reltol, kwargs...)
    @bb δu = similar(u)
    ...
    return DoglegCache{INV, normal_form}(δu, newton_cache, cauchy_cache, internalnorm,
        JᵀJ_cache,  δu_cache_1, δu_cache_2, δu_cache_mul, T(0), T(0))
end

# If TrustRegion is not specified, then use a Gauss-Newton step
function SciMLBase.solve!(cache::DoglegCache{INV, NF}, J, fu; trust_region = Inf,
