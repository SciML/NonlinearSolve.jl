...
# residual is called with are the λspan[1] anchor followed by the accepted continuation
# steps, in order.
λ_targets = Float64[]
...
# expand_factor = 1 reproduces the fixed initial increment: the anchor at λ = 0 plus
# 10 steps of 0.1 (the accumulated 0.1's can undershoot λend by one ulp, costing one
# extra clamped step)
@test 11 <= n_nogrow <= 12
# expansion (defaults: ×2 every 2 successes) must take strictly fewer steps
@test n_grow < n_nogrow

# the expanded increment must never exceed max_step_factor of the span (the leading
# anchor λ = 0 contributes a zero-width diff, which the maximum ignores)
empty!(λ_targets)
